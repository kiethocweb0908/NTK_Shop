...
const CollectionManagement = () => {
  ...
  useEffect(() => {
    ...
    dispatch(fetchAllCollectionsAdmin(filterParams))
      .unwrap()
      .then((result) => {
  ...
  ///
  const toggleActive = async (collectionId) => {
    try {
      const response = await dispatch(toggleActiveThunk({ collectionId })).unwrap();
  ...
  const handleDelete = async (collectionId) => {
    const toastId = toast.loading('Đang xoá bộ sưu tập...', { duration: Infinity });
    try {
      const response = await dispatch(deleteCollectionThunk({ collectionId })).unwrap();
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-semibold uppercase">Quản Lý Bộ sưu tập</h2>
      </div>

      {/* giữa */}
      <div className="mb-4 flex  items-end justify-between">
        {/* Thêm mới */}
        <Link to={`/admin/collections/add`} className="cursor-pointer">
          <Badge
            variant="success"
          ...
          <div>
            ...
            >
              ...
              <SelectContent className="bg-white w-50">
                ...
                <SelectItem className={'hover:bg-gray-100'} key={1} value="published">
                  Đang hoạt động
                </SelectItem>
                <SelectItem className={'hover:bg-gray-100'} key={2} value="hidden">
                  Đã ẩn
                </SelectItem>
              </SelectContent>
            </Select>
      ...
      <div className="overflow-x-auto min-h-[512px] max-h-[512px] overflow-y-auto relative border border-gray-200 rounded-xl flex flex-col justify-between">
        ...
        <table className="min-w-full text-left text-gray-500 select-none">
          <thead className="bg-gray-100 text-xs uppercase text-gray-700 sticky top-0 right-0 left-0 z-5">
            <tr>
              ...
              <th className="py-4 pl-20 pr-4">Tên bộ sưu tập</th>
                ...
                collections.map((collection) => {
                  return (
                    <tr
                      key={collection._id}
                      onClick={() =>
                        navigate(`/admin/products?productCollection=${collection._id}`)
                      }
                      className={`border-b border-gray-300 text-sm text-black/80  cursor-pointer
                      ${!collection.isActive ? 'bg-black/15 opacity-70 hover:opacity-100 transition-all duration-200' : 'hover:bg-gray-50'}`}
                    >
                      {/* Mã đơn */}
                      ...
                      <td className="py-3 px-4 text-center">
                        <Badge
                          variant={collection?.isActive ? 'success' : 'neutral'}
                          className="min-w-[93px]"
                        >
                          {collection?.isActive ? <CheckCircle /> : <EyeOff />}

                          {collection?.isActive ? 'Đang hoạt động' : 'Ẩn'}
                        </Badge>
                      </td>
                      ...
                      >
                        <div className="grid grid-cols-3 gap-2">
                          ...
                          {/* Chỉnh sửa */}
                          <Link
                            to={`/admin/collections/${collection._id}/edit`}
                            className="p-1"
                          >
        ...
        <div className="bg-gray-100 text-xs px-4 py-3 text-gray-700 flex gap-2 sticky bottom-0 left-0 right-0 z-5">
          <div className="text-sm font-medium">Tổng số bộ sưu tập: </div>
          <div className="text-sm font-bold">{collections.length}</div>
        </div>
      </div>
